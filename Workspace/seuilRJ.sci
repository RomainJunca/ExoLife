function img_out = seuilRJ(img, seuil)
    ...
    for Y = 1:size_Y,   //On parcourt la matrice en évitant les extrémités.
        for X = 1:size_X,
            if img(X, Y) <= seuil then  //SI la valeur du pixel est inférieure à la valeur du seuil affecté.
